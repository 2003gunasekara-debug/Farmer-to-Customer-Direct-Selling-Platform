...
const FarmerOrders = () => {
  ...
  const handleAdvance = async (order) => {
    ...
    if (!canRoleUpdateToStatus('farmer', nextStatus)) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        <h2 className="text-xl font-semibold">My Order Status Management</h2>
      ...
      {!loading && !sortedOrders.length && <p className="text-sm text-gray-500">No customer orders yet.</p>}
      ...
      {sortedOrders.map((order) => {
        ...
        const nextStatus = getNextOrderStatus(order);
        const canAdvance = canRoleUpdateToStatus('farmer', nextStatus);
